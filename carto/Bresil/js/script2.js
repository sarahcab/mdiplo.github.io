...
window.onload = function(){
	bout_l.onclick = function(){
		...
		d3.select("#inner").attr("class","pspi").style("opacity",1).selectAll(".survool").remove();
		
		move(inner.offsetLeft + 42.5);
		initialize();	
	}
...
//fonction princiaple
function initialize() {	
	var pos = inner.offsetLeft + 42.5;
	var scrollAnc = 0;
	window.onscroll = function(){
		var scrollAnc = 0;
		var scrollPo = window.scrollY;
		//alert(scrollPo);
		if(scrollPo>scrollAnc){
			pos = pos + 30;
		} else {
			pos = pos - 30;
		}
...
function move(eX){
	...
	d3.select("#visuel").attr("viewBox","0 0 "+nX+" 690").attr("width",prc*taille+"%")
	...
	//test
	var test = document.getElementById("date").offsetHeight;
	d3.select("#slt").html(test);
 }
